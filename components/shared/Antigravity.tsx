...
const AntigravityInner: React.FC<AntigravityProps> = ({
  ...
  useFrame((state) => {
    ...
    const pointer = globalMouse.current ?? m;
    ...
    const isMoving =
      ...
      Date.now() - lastMouseMoveTime.current < idleTimeout;

    const targetActivity = hideWhenIdle ? (isMoving ? 1.0 : 0.0) : 1.0;
    activityRef.current += (targetActivity - activityRef.current) * (isMoving ? 0.2 : 0.05);
    const activity = activityRef.current;

    // When cursor is idle and fully faded, skip instance updates & rendering
    if (hideWhenIdle && activity < 0.005 && !autoAnimate) {
      if (mesh.count !== 0) {
        mesh.count = 0;
    ...
    particles.forEach((particle, i) => {
      ...
      if (dist < magnetRadius) {
        ...
        targetPos.x = THREE.MathUtils.lerp(mx, ringX, activity);
        targetPos.y = THREE.MathUtils.lerp(my, ringY, activity);
        targetPos.z = THREE.MathUtils.lerp(mz * depthFactor, ringZ, activity);
      }
